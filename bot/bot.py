...
async def complete_click_inline(call):
    ...
    data = {"id": user_id, "filters": user_filters[user_id]}
...
async def change_filters(message):
    ...
    for _filter in news_filters[:8]:
        if _filter in existing_filters:
            i += 1
            buttons.append(
                types.InlineKeyboardButton(text=f"{_filter} ✅", callback_data=_filter)
            )
        else:
            i += 1
            buttons.append(
                types.InlineKeyboardButton(text=_filter, callback_data=_filter)
            )
        if i == 2:
            i = 0
            markup.row(*buttons)
...
async def subscribes(message):
    ...
    await bot.send_message(
        message.chat.id,
        "Вы можете добавить каналы, с которых Вам будут приходить новости. Данные новости обновляются с публикацией их на каланах подписки, а не в стандартной логике.",
        reply_markup=markup,
    )
...
async def send_news(user, is_subscribe=False):
    ...
    while True:
        ...
        if data["message_id"] in user_history[user_id]:
            ...
            async with aiohttp.ClientSession() as session:
                response = await session.get(
                    url=f'{URL}/api/history/{user_id}/{channel.id}/{data["message_id"]}'
                )
            if response.status == 200:
        ...
        if user.get("filter_words"):
            if any(
                word for word in user["filter_words"] if word in data["text"].lower()
            ):
                continue
